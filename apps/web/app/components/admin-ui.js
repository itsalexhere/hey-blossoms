...
/** Format tanggal produk untuk tabel admin (locale id-ID). */
...
export function PageTitle({ title, subtitle, right }) {
    return (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '1.5rem', gap: '1rem', flexWrap: 'wrap' }}>
            <div>
                <h1 style={{ fontFamily: 'Playfair Display, serif', fontSize: '1.8rem', margin: 0 }}>{title}</h1>
...
export function Button({ children, onClick, variant = 'primary', disabled, type = 'button', style }) {
    const variants = {
        primary: { background: AC.dark, color: '#fff', border: 'none' },
        gold: { background: AC.gold, color: AC.dark, border: 'none' },
...
export function StatusBadge({ status }) {
    ...
    const color = running ? '#2980b9' : ok ? AC.success : AC.danger;
    return (
        <span
            style={{
